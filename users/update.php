...
if (isset($_POST['cambio']))
{
    $email = ($_POST['email']);
    $doc = ($_POST['doc']);
     $sql1 = $con->prepare("SELECT count(email) FROM user WHERE email = '$email' AND document = $doc LIMIT 1");
     $sql1->execute();
     if($sql1->fetchColumn() > 0)
     {
          $sqlemail = $con->prepare("SELECT * FROM user WHERE email = '$email' AND document = $doc LIMIT 1");
          $sqlemail->execute();
                                ...
                                <div class="mb-4">
                                    <div class="form-outline mb-4">
                                        <label for="contra" class="nav-link"> &nbsp;<strong>ESCRIBA SU CONTRASEÑA </strong></label>
                                            <input type="hidden" id="doc" value="<?php echo $row['document'] ?>" class="form-control Input" name="doc">
