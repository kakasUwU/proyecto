...
$sql1 = $con->prepare("SELECT * FROM oferta LEFT OUTER JOIN municipios ON oferta.id_ciu = municipios.id_municipio LEFT OUTER JOIN depar ON municipios.departamento_id = depar.id_depart WHERE oferta.estado_empleo = 'Activo' AND oferta.cargo = 'Directivo' ORDER BY cargo LIMIT $empieza, $por_pagina");
$sql1->execute();
...
<body>
    ...
    <div class="container-fluid" style="border-top: 1px solid #E1E1E1; padding: 20px; ">
      ...
      <div>
                <?php  
                    $sql = $con->prepare("SELECT COUNT(*) FROM oferta LEFT OUTER JOIN municipios ON oferta.id_ciu = municipios.id_municipio LEFT OUTER JOIN depar ON municipios.departamento_id = depar.id_depart WHERE oferta.estado_empleo = 'Activo' AND oferta.cargo = 'Directivo' ORDER BY cargo");
                    $sql->execute();
                    ...
                    {
                        echo "<center><a href='empleo_directivo.php?pagina=1'>" . "<i class='fa fa-arrow-left'></i>" . "</a>";
                        for ($i = 1; $i <= $total_paginas; $i++) 
                        {
                            echo "<a href='empleo_directivo.php?pagina=" . $i . "'> " . $i . " </a>";
                        }
                        echo "<a href='empleo_directivo.php?pagina=$total_paginas'>" . "<i class='fa fa-arrow-right'></i>" . "</a></center>";
                    }
                ?>
