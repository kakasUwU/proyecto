...
$sql1 = $con->prepare("SELECT * FROM oferta LEFT OUTER JOIN municipios ON oferta.id_ciu = municipios.id_municipio LEFT OUTER JOIN depar ON municipios.departamento_id = depar.id_depart WHERE oferta.estado_empleo = 'Activo' AND oferta.educacion = 'Profesional' ORDER BY cargo LIMIT $empieza, $por_pagina");
$sql1->execute();
    ...
    <div class="container-fluid" style="border-top: 1px solid #E1E1E1; padding: 20px; ">
                <div class="row gy-5 gx-4">
                    <div class="row row-cols-1 row-cols-sm-2 row-cols-md-4 g-3">
                            ...
                            <div class="nav-item dropdown">
                            <a href="#" class="nav-link dropdown-toggle " data-bs-toggle="dropdown"><i class="fas fa-graduation-cap" >&nbsp;Estudios Requeridos</a></i>
                                <div class="dropdown-menu bg-light m-0">
                                    ...
                                    <a href="empleo_profesional.php" class="dropdown-item active">Profesional</a>
                                </div>
                            </div>
                        </div>
                    </div>
      ...
      <div>
                <?php  
                    $sql = $con->prepare("SELECT COUNT(*) FROM oferta LEFT OUTER JOIN municipios ON oferta.id_ciu = municipios.id_municipio LEFT OUTER JOIN depar ON municipios.departamento_id = depar.id_depart WHERE oferta.estado_empleo = 'Activo' AND oferta.educacion = 'Profesional' ORDER BY cargo");
                    $sql->execute();
                    ...
                    {
                        echo "<center><a href='empleo_profesional.php?pagina=1'>" . "<i class='fa fa-arrow-left'></i>" . "</a>";
                        for ($i = 1; $i <= $total_paginas; $i++) 
                        {
                            echo "<a href='empleo_profesional.php?pagina=" . $i . "'> " . $i . " </a>";
                        }
                        echo "<a href='empleo_profesional.php?pagina=$total_paginas'>" . "<i class='fa fa-arrow-right'></i>" . "</a></center>";
                    }
                ?>
