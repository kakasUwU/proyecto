...
$sql1 = $con->prepare("SELECT * FROM oferta LEFT OUTER JOIN municipios ON oferta.id_ciu = municipios.id_municipio LEFT OUTER JOIN depar ON municipios.departamento_id = depar.id_depart WHERE oferta.estado_empleo = 'Activo' AND oferta.cargo = 'Tecnicas de Ejecucion' ORDER BY oferta.cargo LIMIT $empieza, $por_pagina");
$sql1->execute();
...
<body>
    ...
    <br>
      ...
      <div>
                <?php  
                    $sql = $con->prepare("SELECT COUNT(*) FROM oferta LEFT OUTER JOIN municipios ON oferta.id_ciu = municipios.id_municipio LEFT OUTER JOIN depar ON municipios.departamento_id = depar.id_depart WHERE oferta.estado_empleo = 'Activo' AND oferta.cargo = 'Tecnicas de Ejecucion' ORDER BY oferta.cargo");
                    $sql->execute();
                    ...
                    {
                        echo "<center><a href='empleo_ejecucion.php?pagina=1'>" . "<i class='fa fa-arrow-left'></i>" . "</a>";
                        for ($i = 1; $i <= $total_paginas; $i++) 
                        {
                            echo "<a href='empleo_ejecucion.php?pagina=" . $i . "'> " . $i . " </a>";
                        }
                        echo "<a href='empleo_ejecucion.php?pagina=$total_paginas'>" . "<i class='fa fa-arrow-right'></i>" . "</a></center>";
                    }
                ?>
