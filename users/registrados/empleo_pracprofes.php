...
$sql1 = $con->prepare("SELECT * FROM oferta LEFT OUTER JOIN municipios ON oferta.id_ciu = municipios.id_municipio LEFT OUTER JOIN depar ON municipios.departamento_id = depar.id_depart WHERE oferta.estado_empleo = 'Activo' AND oferta.tip_cotra = 'Practica Profesional' ORDER BY cargo LIMIT $empieza, $por_pagina");
$sql1->execute();
...
<body>
    ...
    <div class="container-fluid" style="border-top: 1px solid #E1E1E1; padding: 20px; ">
                    ...
                    <div class="row row-cols-1 row-cols-sm-2 row-cols-md-4 g-3">
                            ...
                            <a href="#" class="nav-link dropdown-toggle " data-bs-toggle="dropdown"><i class="fas fa-book" >&nbsp;Tipo Contrato</a></i>
                                <div class="dropdown-menu bg-light m-0">
                                    ...
                                    <a href="empleo_pracprofes.php" class="dropdown-item active">Practica Profesional</a>
                                </div>
                            </div>
                            <div class="nav-item dropdown">
      ...
      <div>
                <?php  
                    $sql = $con->prepare("SELECT COUNT(*) FROM oferta LEFT OUTER JOIN municipios ON oferta.id_ciu = municipios.id_municipio LEFT OUTER JOIN depar ON municipios.departamento_id = depar.id_depart WHERE oferta.estado_empleo = 'Activo' AND oferta.tip_cotra = 'Productiva Sena' ORDER BY cargo");
                    $sql->execute();
                    ...
                    {
                        echo "<center><a href='empleo_pracprofes.php?pagina=1'>" . "<i class='fa fa-arrow-left'></i>" . "</a>";
                        for ($i = 1; $i <= $total_paginas; $i++) 
                        {
                            echo "<a href='empleo_pracprofes.php?pagina=" . $i . "'> " . $i . " </a>";
                        }
                        echo "<a href='empleo_pracprofes.php?pagina=$total_paginas'>" . "<i class='fa fa-arrow-right'></i>" . "</a></center>";
                    }
                ?>
