...
$sql1 = $con->prepare("SELECT * FROM user LEFT OUTER JOIN tip_user ON user.id_tip_user = tip_user.id_tip_user WHERE tip_user.id_tip_user = 2 ORDER BY activo LIMIT $empieza, $por_pagina");
$sql1->execute();
 ...
 <nav class="navbar navbar-expand-lg bg-white navbar-light sticky-top py-0 pe-5">
        ...
        <div class="collapse navbar-collapse" id="navbarCollapse">
            ...
            <div class="nav-item dropdown">
                    <a href="#" class="nav-link dropdown-toggle active" data-bs-toggle="dropdown">Usuarios</a>
                    <div class="dropdown-menu bg-light m-0">
                        <a href="activar_usuarios.php" class="dropdown-item active">Activar Usuarios</a>
            ...
            <?php foreach ($resultado1 as $row) { ?>
                <div class="card shadow-sm">
                    <div class="row">
                        ...
                        <div class="col">
                            <img src="../../img/proyectos/user/<?php echo $row['image']; ?>" width="50" height="50"></img>
                        </div>
                        <div class="col-1">
                            <p style="font-size: 15px">
                                <a title="ACTIVAR USUARIO" href="active.php?ids=<?php echo $row['document']; ?>&token=<?php echo hash_hmac('sha1', $row['document'], KEY_TOKEN); ?>" class="btn btn-secondary" onclick="alerta('ALERTA!!! VA A ACTIVAR AL USUARIO!!!')" ><i class="fas fa-toggle-on"></i></a>
                                <a title="INACTIVAR USUARIO" href="inactive.php?ids=<?php echo $row['document']; ?>&token=<?php echo hash_hmac('sha1', $row['document'], KEY_TOKEN); ?>" class="btn btn-outline-success" onclick="alerta('ALERTA!!! VA A INACTIVAR AL USUARIO!!!')" ><i class="fas fa-toggle-off"></i></a>
                            </p>
    ...
    <div>
                <?php  
                    $sql = $con->prepare("SELECT COUNT(*) FROM user LEFT OUTER JOIN tip_user ON user.id_tip_user = tip_user.id_tip_user WHERE tip_user.id_tip_user = 2 ORDER BY activo");
                    $sql->execute();
                    ...
                    {
                        echo "<center><a href='activar_usuarios.php?pagina=1'>" . "<i class='fa fa-arrow-left'></i>" . "</a>";
                        for ($i = 1; $i <= $total_paginas; $i++) 
                        {
                            echo "<a href='activar_usuarios.php?pagina=" . $i . "'> " . $i . " </a>";
                        }
                        echo "<a href='activar_usuarios.php?pagina=$total_paginas'>" . "<i class='fa fa-arrow-right'></i>" . "</a></center>";
                    }
                ?>
